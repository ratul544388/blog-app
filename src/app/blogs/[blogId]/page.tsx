...
import BlogDropdownMenu from "@/components/blogs/blog-dropdown-menu";
...
import { BlogVotes } from "../_components/votes/blog-votes";
...
const Page = async ({ params }: { params: { blogId: string } }) => {
  ...
  const similarBlogs = await db.blog.findMany({
    where: {
      id: {
        not: blog.id,
      },
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-8 gap-8 lg:gap-12">
        <Description
          ...
          className="text-foreground col-span-8 md:col-span-5"
        />
        <AsideBlogs blogs={similarBlogs} about="" title="" />
      </div>
